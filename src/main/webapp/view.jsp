...
<body>
<% Complaints c= (Complaints)session.getAttribute("complaint"); %>
  <div class="container">
    <h2> Complaint</h2>
      ...
      <input type="text" i name="flat" value="<%= c.getFlatNo() %>" readonly >

      <label for="email">Email</label>
      <input type="email"  name="email"  value="<%= c.getEmail() %>" readonly  >

      <label for="phone">Phone Number</label>
      <input type="tel" name="phone" value="<%= c.getMobile() %>" readonly >

      <label for="type">Complaint Type</label>
      <input type="text" value="<%= c.getComplaintType() %>" readonly>

      <label for="urgency">Urgency Level</label>
      <input type="text" value="<%= c.getUrgencyLevel() %>" readonly>

      <label for="desc">Complaint Description</label>
      <textarea id="desc" name="desc" ><%= c.getComplaintDesp() %></textarea>

      <a href="userportal" ><button>Home</button></a>
  </div>
</body>
</html>